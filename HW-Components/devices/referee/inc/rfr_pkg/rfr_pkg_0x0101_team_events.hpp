...
struct __REFEREE_PACKED TeamEventData {
  uint32_t restoration_1 : 1;  ///< bit 0: 己方与兑换区不重叠的补给区占领状态，1 为已占领
  uint32_t restoration_2 : 1;  ///< bit 1: 己方与兑换区重叠的补给区占领状态，1 为已占领
  /** bit 2: 己方补给站的占领状态，1 为已占领 @attention 仅 RMUL 适用 */
  uint32_t supplier_area : 1;
  uint32_t small_power_rune : 1;  ///< bit 3: 己方小能量机关的激活状态，1 为已激活
  uint32_t large_power_rune : 1;  ///< bit 4: 己方大能量机关的激活状态，1 为已激活
  /** bit 5-6: 己方中央高地的占领状态，1 为被己方占领，2 为被对方占领 */
  uint32_t central_highland : 2;
  /** bit 7-8: 己方梯形高地的占领状态，1 为已经占领*/
  uint32_t trapezoid_highland : 2;
  /**
   * bit 9-17:
   * 对方飞镖最后一次击中己方前哨站或基地的时间
   * 0-420，开局默认为 0
   */
   ...
   * 开局默认为 0, 1 为击中前哨站，2 为击中基地固定目标，3 为击中基地随机固定目标，4 为击中基地随机移动目标
   */
  uint32_t dart_last_hit_us_type : 3;
  /**
   * bit 21-22:
   * 中心增益点的占领情况
...
class TeamEventPackage : public ProtocolRxPackage
{
 public:
  ...
  /**
   * @brief 己方中央高地的占领状态，1 为被己方占领，2 为被对方占领 
   */
  uint8_t central_highland(void) const { return data_.central_highland; }
  /**
   * @brief 己方梯形高地的占领状态，1 为已占领 
   */
  uint8_t trapezoid_highland(void) const { return data_.trapezoid_highland; }
  /**
   ...
   *        为击中基地固定目标，3 为击中基地随机固定目标, 4 为击中基地随机移动目标 
   */
