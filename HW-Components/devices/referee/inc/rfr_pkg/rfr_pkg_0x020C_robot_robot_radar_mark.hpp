 ...
 * @brief 雷达机器人标记进度数据
 * 在对应机器人被标记进度 ≥ 100 时发送 1
 * 被标记进度 < 100 时发送 0。 
 */
struct __REFEREE_PACKED RobotRadarMarkData {
  uint8_t hero : 1;        ///< 对方 1 号英雄机器人易伤情况：0—120
  uint8_t engineer : 1;    ///< 对方工程机器人被标记进度：0—120
  uint8_t standard_3 : 1;  ///< 对方 3 号步兵机器人被标记进度：0—120
  uint8_t standard_4 : 1;  ///< 对方 4 号步兵机器人被标记进度：0—120
  uint8_t sentry : 1;      ///< 对方哨兵机器人被标记进度：0—120
  uint8_t reserved : 3;    ///< 保留位
};
static_assert(sizeof(RobotRadarMarkData) == 1, "RobotRadarMarkData size error");
...
class RobotRadarMarkPackage : public ProtocolRxPackage
{
 public:
  ...
  /**
   * @brief 对方 1 号英雄机器人易伤情况
   */
  bool hero(void) const { return data_.hero; }
  /**
   * @brief 对方工程机器人被标记进度
   */
  bool engineer(void) const { return data_.engineer; }
  /**
   * @brief 对方 3 号步兵机器人被标记进度
   */
  bool standard_3(void) const { return data_.standard_3; }
  /**
   * @brief 对方 4 号步兵机器人被标记进度
   */
  bool standard_4(void) const { return data_.standard_4; }
  /**
   * @brief 对方哨兵机器人被标记进度
   */
  bool sentry(void) const { return data_.sentry; }
