...
class Vision : public comm::Receiver, public comm::Transmitter
{
 public:
  ...
  struct TxData : public MemMgr {
    ...
    float blt_spd = 22.0f;  ///< 发送给视觉的子弹速度，单位：m/s
    uint8_t tgt_id = 0u;    ///< 目标 ID (无人机使用，0 不筛选，1 只打前哨站，2 只打基地)
  };
  ...
  struct RxData : public MemMgr {
    ...
    float distance = 0.0f;  ///< 瞄准目标距离，单位：m
  };

  struct Config : public MemMgr {
    float default_blt_spd = 22.0f;     ///< 默认子弹速度，单位：m/s
  ...
  static constexpr uint32_t kRxDataLen = 20;  ///< 电控接收/视觉发送数据长度
  static constexpr uint32_t kTxDataLen = 20;  ///< 电控发送/视觉接收数据长度
  ...
  {
    return (AngleNormRad(fabsf(rx_data_.pitch - pitch)) < half_vfov_) &&
           (AngleNormRad(fabsf(rx_data_.yaw - yaw)) < half_hfov_);
  }
  ...
  bool setTgtId(uint8_t tgt_id)
  {
    if (tx_data_.tgt_id == tgt_id) {
      return false;
    }
    tx_data_.tgt_id = tgt_id;
    return true;
  }
  uint8_t getTgtId(void) const { return tx_data_.tgt_id; }
  ...
  uint16_t getVtmX(void) const { return rx_data_.vtm_x; }

  uint16_t getVtmY(void) const { return rx_data_.vtm_y; }

  float getDistance(void) const { return rx_data_.distance; }
