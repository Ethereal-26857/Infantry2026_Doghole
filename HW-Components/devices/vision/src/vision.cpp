...
bool Vision::encode(size_t &len, uint8_t *data)
{
  ...
  // 计算校验位，将数据缓冲区的第3个字节到倒数第2个字节进行 CRC8 校验得到校验位
  tx_data_buffer_[kTxDataLen - 1] = crc8_.getCrc8CheckNum(tx_data_buffer_ + 2, kTxDataLen - 3, 0);
...
bool Vision::decode(size_t len, const uint8_t *data, uint32_t rx_id)
{
  n_rx_frames_ = 0;
  if (len != kRxDataLen) {
    decode_fail_cnt_++;
  ...
  switch (rx_status_) {
    ...
    case RxStatus::kWaitingTailCplt: {
      ...
      do {
        ...
        if (rx_data_buffer_[kRxDataLen - 2] != crc8_.getCrc8CheckNum(rx_data_buffer_ + 3, kRxDataLen - 5, 0)) {
...
void Vision::decodeRxData(size_t len, const uint8_t *data)
{
  ...
  rx_data_.vtm_x = static_cast<uint16_t>(data[9] << 8 | data[8]);
  rx_data_.vtm_y = static_cast<uint16_t>(data[11] << 8 | data[10]);
  rx_data_.distance = static_cast<float>(data[12]) / 10.0f;
};
