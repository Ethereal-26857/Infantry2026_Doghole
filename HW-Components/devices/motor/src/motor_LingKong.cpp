...
LingKong::LingKong(uint8_t id, const OptionalParams& opt)
...
bool LingKong::decode(size_t len, const uint8_t* data, uint32_t rx_id)
{
  /* 变量检查 */
#pragma region
...
bool LingKong::encode(size_t& len, uint8_t* data)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(data, "data is nullptr");
...
#pragma endregion
  ...
  int16_t input =
      Bound(static_cast<float>(motor_info_.dir) * raw_input_,
...
void LingKong::init(uint8_t id, const OptionalParams& opt)
{
  /* 变量检查 */
#pragma region
  HW_ASSERT(1 <= id && id <= 4, "Error id: %d", id);
