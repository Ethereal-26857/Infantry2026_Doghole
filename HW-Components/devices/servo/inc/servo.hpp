...
struct ServoInfo : public MemMgr {
  ...
  ServoDirection direction = kCW;   ///* 舵机转动方向
  float angle_offset = 0;                 ///* 舵机零点偏移,默认以行程中值为零点，单位
};
...
class Servo : public MemMgr
{
 public:
  ...
  /**
   * @brief       设定舵机角度零位
   * @param        angle: 舵机角度零位，单位：deg
   * @retval       None
   * @note        舵机角度基于零位调整
   */
  void setAngleOffset(float angle_offset)
  {
    servo_info_.angle_offset = angle_offset;
  }
 ...
 private:
  ...
  {
    angle *= static_cast<float>(servo_info_.direction);
    angle += servo_info_.angle_offset;
    angle += servo_info_.angle_range / 2.0f;
