...
struct FricConfig2Motor {
  ...
  uint32_t resurrection_duration = 2000;     // 复活模式持续时间, >0, 默认值 2000 ms
  /* 优化项，建议开启 */
...
class Fric : public ModuleFsm
{
 public:
  ...
  /* 读取内部数据 */
  [[nodiscard]] bool getStatus() const noexcept { return status_.is_ready; }  // 摩擦轮是否准备好发弹
  [[nodiscard]] Config &getConfig() noexcept { return cfg_; }
  [[nodiscard]] const Config &getConfig() const noexcept { return cfg_; }
  [[nodiscard]] WorkingMode getWorkingMode() const noexcept { return working_mode_; }
 ...
 private:
  ...
  /* 校验配置函数 */
  Config validateConfig(const Config &config);
  Config cfg_;
