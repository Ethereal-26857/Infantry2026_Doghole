...
/* 对于一定尺寸的拨弹盘，确定电机零位和角度偏移量后，其每发拨弹角度都唯一确定。
   遇到前向堵转时，有两种方法处理：其一，回退到最近的上一发拨弹角度。
   在某些情况下，用户认为该方法回退角度过小，不足以解决堵转问题。
   此时可以选择第二种方法，回退一个固定角度，该固定角度通常大于 1 发弹丸对应的角度。
!!!【注意】选用第二种方法时，应当要求拨弹盘机械设计合理，保证不会造成反向堵转 !!!! 
   （除了前向堵转，还可能有后向堵转，但该情况比较少见，处理方法一般是直接往前转一点点。）*/
...
/* 针对【单发工作情况】，只要拨弹盘机械设计合理，可以每拨一发弹后都回退一个小角度，
   以避免弹链过紧造成堵转 */
...
class Feed : public ModuleFsm
{
 public:
  /* 类型定义 */
  using Pid = pid::MultiNodesPid;
  using Motor = motor::Motor;
  using ShootFlag = vision::Vision::ShootFlag;
  ...
  Config validateConfig(const Config &config);                             // 校验配置
  void setConfig(const Config &config) { cfg_ = validateConfig(config); }  // 设定配置
  ...
  /* 读取内部数据 */
  [[nodiscard]] Config &getConfig() noexcept { return cfg_; }              // 获取当前配置
  [[nodiscard]] const Config &getConfig() const noexcept { return cfg_; }  // 获取当前配置 (适用于 const 实例)
  [[nodiscard]] CtrlMode getCtrlMode() const noexcept { return ctrl_mode_; }
  [[nodiscard]] StuckStatus getStuckStatus() const noexcept { return status_.stuck_status; }
  [[nodiscard]] float getAngRef() const noexcept { return status_.ang_ref; }
  [[nodiscard]] float getAngFdb() const noexcept { return status_.ang_fdb; }
  [[nodiscard]] uint16_t getTriggerCnt() const noexcept { return trigger_cnt_; }
  [[nodiscard]] uint16_t getStuckForwardCnt() const noexcept { return stuck_forward_cnt_; }
  [[nodiscard]] uint16_t getStuckBackwardCnt() const noexcept { return stuck_backward_cnt_; }
  /* 注册组件指针 */
 ...
 private:
  ...
  void updateStuckStatusByAngle();
  void updateStuckStatusByCurrent();
  ...
  struct FeedStatus {
    ...
    float stuck_handle_angle;                       // 堵转处理角度
    bool is_stuck_handled = true;                   // 堵转处理是否完成
    ...
    bool prevent_roll_back_finished = false;  // 预防堵转回退是否完成
