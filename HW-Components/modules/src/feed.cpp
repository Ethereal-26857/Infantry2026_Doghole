...
#pragma region 构造函数
Feed::Feed(const Config &config) : cfg_(validateConfig(config))
{
  trigger_interval_ = config.default_trigger_interval;
  safe_num_bullet_ = config.default_safe_num_blt;
}

Feed::Config Feed::validateConfig(const Config &config)  // 校验配置
{
  ...
  HW_ASSERT(config.stuck_detection.detection_by == StuckDetectionBy::kAngle ||
                config.stuck_detection.detection_by == StuckDetectionBy::kCurrent,
            "Unknown stuck detection by");
  if (config.stuck_detection.detection_by == StuckDetectionBy::kAngle) {
    HW_ASSERT(config.stuck_detection.ang_diff_thre > 0, "ang_diff_thre must be greater than 0");
  } else if (config.stuck_detection.detection_by == StuckDetectionBy::kCurrent) {
    HW_ASSERT(config.stuck_detection.curr_thre > 0, "curr_thre must be greater than 0");
  }
  HW_ASSERT(config.stuck_detection.time_thre > 0, "time_thre must be greater than 0");
  if (config.stuck_handle.method == FeedStuckBackMethod::kFixedAng) {
    HW_ASSERT(config.stuck_handle.back_ang > 0, "back_ang must be greater than 0");
  }
  if (config.stuck_prevent.enabled) {
    HW_ASSERT(config.stuck_prevent.back_ang > 0, "back_ang must be greater than 0");
  }
 ...
 * @note        这里的“保持”是指拨弹盘角度变化范围在 0.001 rad以内，
 *              这个阈值暂时不打算暴露给用户进行配置。
 */
...
/** 
 * @brief       更新电机堵转状态
 * @note        根据配置参数，选择用角度反馈或者电流反馈判断是否堵转
 */
void Feed::updateMotorStuckStatus()
{
  if (cfg_.stuck_detection.detection_by == StuckDetectionBy::kAngle) {
    updateStuckStatusByAngle();  // 用角度反馈判断堵转
  } else if (cfg_.stuck_detection.detection_by == StuckDetectionBy::kCurrent) {
    updateStuckStatusByCurrent();  // 用电流反馈判断堵转
  }
  if (!status_.is_stuck_handled) {
    if (fabsf(AngleNormRad(status_.ang_fdb - status_.stuck_handle_angle)) < 0.002) {
      status_.is_stuck_handled = true;
    }
  }
};

/** 
 * @brief       用角度反馈数据更新电机堵转状态，仅适用于小弹丸拨弹盘电机
 * @note        根据实际角度期望值与反馈角度之差是否长时间超过阈值，判断电机是否堵转
 */
void Feed::updateStuckStatusByAngle()
{
  float diff = AngleNormRad(status_.ang_ref - status_.ang_fdb);
  if (diff > cfg_.stuck_detection.ang_diff_thre) {
    status_.stuck_duration += interval_ticks_;
    if (status_.stuck_duration >= cfg_.stuck_detection.time_thre &&
        status_.ang_hold_duration >= cfg_.stuck_detection.time_thre) {
      status_.stuck_status = StuckStatus::kForward;
      stuck_forward_cnt_++;
    }
  } else if (diff < -cfg_.stuck_detection.ang_diff_thre) {
    status_.stuck_duration += interval_ticks_;
    if (status_.stuck_duration >= cfg_.stuck_detection.time_thre &&
        status_.ang_hold_duration >= cfg_.stuck_detection.time_thre) {
...
/** 
 * @brief       用电流反馈数据更新电机堵转状态，适用于大弹丸拨弹盘电机
 * @note        根据电机反馈电流是否持续超阈值，判断电机是否卡弹
 */
void Feed::updateStuckStatusByCurrent()
{
  if (status_.cur_fdb <= -cfg_.stuck_detection.curr_thre) {
    status_.stuck_duration += interval_ticks_;
    if (status_.stuck_duration >= cfg_.stuck_detection.time_thre) {
      status_.stuck_status = StuckStatus::kBackward;
      stuck_backward_cnt_++;
    }
  } else if (status_.cur_fdb >= cfg_.stuck_detection.curr_thre) {
    status_.stuck_duration += interval_ticks_;
    if (status_.stuck_duration >= cfg_.stuck_detection.time_thre) {
      status_.stuck_status = StuckStatus::kForward;
      stuck_forward_cnt_++;
...
void Feed::standby()
{
  ...
  status_.resetHoldStatus();
  status_.resetStuckStatus();
  status_.prevent_roll_back_finished = false;
  // 为了实现拨弹盘退出待机时的无扰切换
  status_.ang_ref = status_.ang_fdb;
  status_.is_ang_ref_on_grid = false;  // 角度期望值不在网格上
  status_.control_input = 0;
...
{
  // 卡弹未处理完成或用户不允许拨弹时不能生成拨弹信号
  if (!status_.is_stuck_handled || !is_trigger_allowed_) {
 ...
 * @retval      是否允许拨弹
 */
bool Feed::limitTriggerByFreq(uint32_t interval) const
{
  return (work_tick_ - last_trigger_tick_) > interval;
};
...
void Feed::calcFakeHeat()
{
  if (rfr_data_.is_rfr_on) {
    ...
    // 为防止 fake_heat_ 与裁判系统热量差距过大，以裁判系统热量为准
    if (rfr_data_.heat - fake_heat_ > 20.0f) {
      fake_heat_ = rfr_data_.heat;
    }
  } else {
...
void Feed::calcAngRefOnResurrection()
{
  ...
  if (status_.stuck_status == StuckStatus::kBackward || status_.ang_hold_duration > cfg_.hold_duration_thre) {
    ...
    status_.is_ang_ref_on_grid = true;      // 角度期望值在网格上
    status_.resetStuckStatus();
  }
};
...
{
  if (status_.stuck_status == StuckStatus::kForward) {
    if (cfg_.stuck_handle.method == StuckBackMethod::kToLastAng) {
      status_.ang_ref = searchAngRef(status_.ang_fdb, false);  // 拨盘前向堵转时，回到上一个最近期望角度
    } else if (cfg_.stuck_handle.method == StuckBackMethod::kFixedAng) {
      status_.ang_ref = AngleNormRad(status_.ang_fdb - cfg_.stuck_handle.back_ang);
      status_.is_ang_ref_on_grid = false;  // 角度期望值不在网格上
    }
    status_.resetStuckStatus();  // 清空堵转计时器，给电机充分时间回退；清除前向堵转标志位，避免连续回退
    status_.is_stuck_handled = false;
    status_.stuck_handle_angle = status_.ang_ref;
    return;
  }
  if (status_.stuck_status == StuckStatus::kBackward) {
    status_.ang_ref = AngleNormRad(status_.ang_fdb + 0.05f);  // 拨盘后向堵转时，将目标角度设置为当前反馈角度略微往前一点
    status_.is_ang_ref_on_grid = false;                       // 角度期望值不在网格上
    status_.resetStuckStatus();                               // 清空堵转计时器，给电机充分时间前向转动；清除后向堵转标志位，避免连续回退
    status_.is_stuck_handled = false;
    status_.stuck_handle_angle = status_.ang_ref;
    return;
  }

  /* 非堵转情况下，才会生成trigger_signal_，进行拨弹 */
  if (trigger_signal_) {
    /* 若开启预防堵转，已经回退，则需注意下次拨弹的期望角度是前向第二近的拨弹角度 */
    if (cfg_.stuck_prevent.enabled && status_.prevent_roll_back_finished) {
      status_.ang_ref = searchAngRef(status_.ang_fdb, true);
      status_.ang_ref = searchAngRef(status_.ang_ref, true);
      status_.prevent_roll_back_finished = false;
    } else {
      /* 
      Q1：为何不统一采用status_.ang_ref = searchAngRef(status_.ang_fdb, true)？is_ang_ref_on_grid是否冗余？
      A1：高频工况下（小弹丸≥20Hz），逐周期根据 ang_fdb 计算 ang_ref 会产生非固定角度差，
          导致拨盘反复加减速。通过is_ang_ref_on_grid标记正常状态，直接对ang_ref进行固定角度步进，
          可维持稳定的角度差（大概率 > ang_per_blt），实现恒定高转速，达到高频拨弹要求。
      Q2：以上问题归根结底是因为我们采用位置速度双环 PID ，若改用速度单环PID控制能否规避此问题？
      A2：是的。
          但是，单速度环控制方案极其容易造成双发，且极不利于我们做热量、频率限制。
      【当前存在的问题】高频转动时，ang_ref 与 ang_fdb 差值大概率大于一颗弹丸角度，
      这导致我们的热量限制是“滞后”的。因此，小弹丸兵种需要将热量限制的弹丸数目调大些。
      */
      if (status_.is_ang_ref_on_grid) {
  ...
  /* 开启了预防堵转，期望角度与反馈角度一致，同时尚未回退，则需要回退 */
  if (cfg_.stuck_prevent.enabled && abs(status_.ang_fdb - status_.ang_ref) > 0.001f &&
      !status_.prevent_roll_back_finished) {
    status_.ang_ref = AngleNormRad(status_.ang_ref - cfg_.stuck_prevent.back_ang);
    status_.prevent_roll_back_finished = true;
  }
};
...
void Feed::calcMotorInput()
{
  ...
  if (motor_ptr_->isOffline()) {
    status_.control_input = 0.0f;
  } else {
    float ref = status_.ang_ref;
    float fdb[2] = {status_.ang_fdb, status_.spd_fdb};
    pid_ptr_->calc(&ref, fdb, nullptr, &status_.control_input);
  }
};
#pragma endregion
