...
#pragma region 构造函数
Fric::Fric(const Config &config) : cfg_(validateConfig(config))
{
...
/** 
 * @brief       校验配置
 * @param        &config:待校验的配置引用
 * @retval      通过校验的配置引用
 */
Fric::Config Fric::validateConfig(const Config &config)
{
  ...
  HW_ASSERT(config.opt_blt_spd_cl.min_reasonable_blt_spd < config.opt_blt_spd_cl.max_reasonable_blt_spd,
            "Minimun reasonable bullet speed need to be less than maximun reasonable bullet speed.");
  ...
  HW_ASSERT(config.opt_blt_spd_cl.min_target_blt_spd < config.opt_blt_spd_cl.max_target_blt_spd,
            "Minimun target bullet speed need to be less than maximun target bullet speed.");
  HW_ASSERT(config.opt_blt_spd_cl.spd_gradient >= 0, "Speed gradient need to be greater than 0.");
  HW_ASSERT(config.resurrection_duration > 0, "Resurrection duration need to be greater than 0.");
  return config;
}
#pragma endregion
#pragma region 数据更新
...
void Fric::updatePwrState()
{
  ...
  if (current_state == PwrState::kDead) {
    ...
    /* 电机缓启动. 对于3508电机 + C620 电调，该功能可用于电调自检；
       对于多级摩擦轮发射机构，可通过调整缓启动时间实现摩擦轮逐级启动，
       避免瞬时电流过大导致 Ammo Booster 供电被切断。
    */
    if (resurrection_tick_ >= cfg_.resurrection_duration) {
 ...
 */
void Fric::updateRfrData(const RfrInputData &inp_data)
{
...
void Fric::updateSpdStatus()
{
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    hold_flag[i] = status_.actual_spd_ref > 0 &&
                   fabsf(spd_fdb_delta) < cfg_.spd_delta_thre &&
  ...
  } else if (working_mode_ != WorkingMode::kShoot || status_.stuck_status != StuckStatus::kNone) {
    /* 完成一次启动后，必须等到摩擦轮卡弹或者特殊模式，才重置 is_ready
       避免拨弹时摩擦轮的拨动对 is_ready 的判断造成影响。 */
...
{
  resurrection_tick_ += interval_ticks_;
};
...
void Fric::calcMotorInput()
{
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    */
    ...
    if (motor_ptr->isOffline()) {
      ...
      float fdb = status_.spd_fdb[i];
      pid_ptr->calc(&ref, &fdb, nullptr, &status_.control_input[i]);
    }
  }
  ...
  if (cfg_.opt_spd_same_pid_enabled) {
    ...
    status_.control_input[0] += out;
    status_.control_input[1] -= out;
  }
};
#pragma endregion
...
void Fric::reset()
{
  ...
  memset(status_.control_input, 0, sizeof(status_.control_input));
...
void Fric::setCommData(bool is_working)
{
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    if ((!motor_ptr->isOffline()) && is_working) {
      motor_ptr->setInput(status_.control_input[i]);
    } else {
      pid_ptr->reset();
