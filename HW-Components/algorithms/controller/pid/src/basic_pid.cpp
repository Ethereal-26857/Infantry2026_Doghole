                               ...
                               const float ffd_arr[], float out_arr[])
{
  ...
  datas_.last_raw_err = datas_.raw_err;
  ...
  datas_.fdb = fdb;
  datas_.raw_err = params_.period_sub.calc(datas_.ref, datas_.fdb);
  datas_.err = params_.dead_band.calc(datas_.raw_err);
  ...
  if (params_.kd) {
    float diff = params_.diff_previous.calc(
                     ...
                     datas_.raw_err, datas_.last_raw_err) *
                 (params_.kd / datas_.interval_ms);
    datas_.diff = params_.diff_filter.calc(diff, datas_.diff);

    // 死区生效时，不计算微分
    if (fabsf(datas_.err) < fabsf(datas_.raw_err) ||
        fabsf(datas_.last_err) < fabsf(datas_.last_raw_err)) {
      diff = 0;
    }
    out += datas_.diff;
  } else {
